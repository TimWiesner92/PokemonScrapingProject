...
class PkmnSpider(scrapy.Spider):
    ...
    def closed(self, data):
        ...
        df = df[df['evolves_from'].between('0001', '0251') & ~df['evolves_from'].str.match(r'\d{4}[a-z]')]
        df = df[df['evolves_to'].between('0001', '0251') & ~df['evolves_to'].str.match(r'\d{4}[a-z]')]
        ...
        save_xlsx('pkmn_g2_evo.xlsx')
        df.to_sql('evo', engine, index=False)
        #df.to_csv('pkmn_g2_evo.csv', index=False)
