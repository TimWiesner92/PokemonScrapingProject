...
class PkmnSpider(scrapy.Spider):
    ...
    def closed(self, data):
        ...
        df.loc[len(df)] = ['0082','0462','level','','','','','','Route 13','','','','','X, Y','','']
        df.loc[len(df)+1] = ['0133','0470','level','','','','','','Route 20','','','','','X, Y','','']
        df.loc[len(df)+2] = ['0133','0471','level','','','','','','Frost Cavern','','','','','X, Y','','']
        df.loc[len(df)+3] = ['0299','0476','level','','','','','','Route 13','','','','','X, Y','','']
        df.loc[len(df)+4] = ['0349','0350','level','','','','Beauty','','','','','','','','','']
        ...
        duplicate_mask = df.duplicated(subset=['evolves_from','evolves_to','stat','location'], keep='first')
        ...
        df.loc[(df['evolves_to'] == '0350') & (df['method'] == 'trade'), 'item_held'] = 'Prism Scale'
        ...
        condition_mover('level up in a Magnetic Field area','location','New Mauville')
        condition_mover('level up near an Icy Rock','location','Shoal Cave (ice room, low tide)')
        condition_mover('level up near a Mossy Rock','location','Petalburg Woods')
        ...
        df.loc[df['location'] == 'New Mauville', ['method','item_used','versions']] = ['level','','Omega Ruby, Alpha Sapphire']
        df.loc[df['location'] == 'Shoal Cave (ice room, low tide)', ['method','item_used','versions']] = ['level','','Omega Ruby, Alpha Sapphire']
        df.loc[df['location'] == 'Petalburg Woods', ['method','item_used','versions']] = ['level','','Omega Ruby, Alpha Sapphire']
        ...
        df = df[df['evolves_from'].between('0001', '0721') & ~df['evolves_from'].str.match(r'\d{4}[a-z]$')]
        df = df[df['evolves_to'].between('0001', '0721') & ~df['evolves_to'].str.match(r'\d{4}[a-z]$')]
        ...
        save_xlsx('pkmn_g6_evo.xlsx')
        df.to_sql('evo', engine, index=False)
        #df.to_csv('pkmn_g6_evo.csv', index=False)
