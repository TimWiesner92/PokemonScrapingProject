...
class PkmnSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.serebii.net/attackdex-xy/"]
    ...
    def parse_move_page(self, response):
        ...
        if s.isdigit():
            ...
            effect_rate = s
        a = response.xpath('//td[b[text()="Appeal"]]/ancestor::tr/following-sibling::tr[1]//td[@class="cen"][2]').get()
        j = response.xpath('//td[b[text()="Jam"]]/ancestor::tr/following-sibling::tr[1]//td[@class="cen"][3]').get()
        if a == None:
            contest_type = 'N/A'
            contest_effect = 'N/A'
            appeal = 'N/A'
            ...
            contest_type = response.xpath('//td[b[text()="Contest Type"]]/ancestor::tr/following-sibling::tr[1]//td[@class="cen"]//img[starts-with(@src, "/pokedex-bw/type/")]/@src').get().replace('/pokedex-bw/type/','').replace('.png','').capitalize()
            contest_effect = response.xpath('//td[text()="Contest Effect"]/ancestor::tr/following-sibling::tr[1]//td[@class="fooinfo"]/text()').get().strip()
            if a != ['<td class="cen">\r\n\xa0</td>']:
                appeal = len(a.split('.png"><img'))
            else:
                appeal = 0
            if j != ['<td class="cen">\r\n\xa0</td>']:
                jam = len(j.split('.png"><img'))
            else:
                jam = 0
        ...
        entry = {
            ...
            'sky_battles': response.xpath('//td[contains(text(), "Sky Battles")]/ancestor::tr/following-sibling::tr[1]//td[@class="cen"][5]/text()').get().strip(),
    ...
    def closed(self, data):
        ...
        save_xlsx('pkmn_g6_moves.xlsx')
        df.to_sql('moves', engine, index=False)
        #df.to_csv('pkmn_g6_moves.csv', index=False)
