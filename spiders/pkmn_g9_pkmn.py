...
class PkmnSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.serebii.net/pokedex-sv/"]

    def parse(self, response):
        '''Method for extracting links to pages to crawl from the pulldowns'''
        pulldown_links = [x for x in response.xpath('//select[@name="SelectURL"]')[0:9].css('option').xpath('@value').getall() if x != '#']
    ...
    def parse_move_page(self, response):
        ...
        def parse_gender_ratio(mr,fr):
            ...
            if mr == '0' and fr == '0':
                ratio = 'double_zero (set back to N/A if this shows up, remove this clause if not)'
            elif mr == '0':
                ratio = 'Always female'
            elif fr == '0':
                ratio = 'Always male'
                ...
                ratio = str(reduced_mr) + ':' + str(reduced_fr)
            return ratio
        ...
        def extract_types(html_string, pat1, pat2, pk_name):
            ...
            # Tauros's forms are named differently in Type than they are in Alternate Forms
            if pk_name == 'Tauros':
                if form_match == 'Paldean':
                    form_match = 'Paldean Form Combat Breed'
                elif form_match == 'Blaze Breed':
                    form_match = 'Paldean Form Blaze Breed'
                elif form_match == 'Aqua Breed':
                    form_match = 'Paldean Form Aqua Breed'
            elif pk_name == 'Necrozma' and form_match == 'Normal':
                form_match = 'Necrozma'
            elif pk_name == 'Ursaluna' and form_match == 'Bloodmoon Form':
                form_match = 'Bloodmoon'
            ...
            elif form_match == 'Paldean':
                form_match = 'Paldean Form'
            else:
        ...
        if response.xpath('//td[text()="Gender Ratio"]/ancestor::tr/following-sibling::tr[1]//td[4]').get()[-19:-5] == 'Gender Unknown':
            ...
            if response.xpath('//td[text()="Gender Ratio"]/ancestor::tr/following-sibling::tr[1]//td[4]').get()[91:99] == 'Standard':
                f1 = response.xpath('//td[text()="Gender Ratio"]/ancestor::tr/following-sibling::tr[1]//td[4]').css('td::text').getall()[0].replace('Standard','Normal')
                mr1 = response.xpath('//td[text()="Gender Ratio"]/ancestor::tr/following-sibling::tr[1]//td[4]').css('td::text').getall()[3].replace('%','').replace('*','')
                fr1 = response.xpath('//td[text()="Gender Ratio"]/ancestor::tr/following-sibling::tr[1]//td[4]').css('td::text').getall()[6].replace('%','').replace('*','')
                f2 = response.xpath('//td[text()="Gender Ratio"]/ancestor::tr/following-sibling::tr[1]//td[4]').css('td::text').getall()[7]
                mr2 = response.xpath('//td[text()="Gender Ratio"]/ancestor::tr/following-sibling::tr[1]//td[4]').css('td::text').getall()[10].replace('%','').replace('*','')
                fr2 = response.xpath('//td[text()="Gender Ratio"]/ancestor::tr/following-sibling::tr[1]//td[4]').css('td::text').getall()[13].replace('%','').replace('*','')
                mfr = {f1: parse_gender_ratio(mr1,fr1), f2: parse_gender_ratio(mr2,fr2)}
            else:
        ...
        egg_steps_selector = response.xpath('//td[text()="Base Egg Steps"]/ancestor::tr/following-sibling::tr[1]//td[5]/text()').get().replace(',','').replace(' Steps','')
        ...
        if len(mega_primal_forms) > 0:
            ...
            forms = response.xpath('//td[text()="Alternate Forms"]/ancestor::tr/following-sibling::tr[1]//table[1]//tr[1]/td').xpath('string(.)').getall()
            forms = [i.replace('Kantonian Form','Normal').replace('Johtonian Form','Normal').replace('Hoennian Form','Normal').replace('Unovan Form','Normal').replace('Kalosian Form','Normal').replace('Alolan Form','Normal').replace('Paldean FormA','Paldean Form A').replace('Paldean FormB', 'Paldean Form B').replace('Paldean FormC','Paldean Form C') for i in forms]
            if forms == []:
                forms = ['']
            if name == 'Minior':
                ...
                forms = ['Normal','Bloodmoon']
            elif name in ['Meowstic','Indeedee','Basculegion']:
                forms = ['Male','Female']
        ...
        for form in forms:
            ...
            if len(evs_html) == 1:
                ...
                if name == 'Tauros':
                    if form == 'Normal':
                        evs_sel = ev_split(evs_html)['Normal']
                    else:
                        evs_sel = ev_split(evs_html)['Paldean Form']
                else:
                    evs_sel = ev_split(evs_html)[form]

            # set up selectors for male_female_ratio:
            if type(mfr) == dict:
                mfr_sel = mfr[form]
            else:
                mfr_sel = mfr
...
#                'ability_two': abilities[1] if len(abilities) == 2 else '',
                ...
                'egg_group_one': eg1,
                'egg_group_two': eg2,
                'change_tera': response.xpath('//td[text()="Can Change Tera Type?"]/ancestor::tr/following-sibling::tr[1]//td[4]//text()').get().strip(),
                'hp_evs': evs_sel[0],
                'atk_evs': evs_sel[1],
                'def_evs': evs_sel[2],
    ...
    def closed(self, data):
        ...
        for i in ['0386no','0479ro','0483di','0484pa','0555st','0646ky','0800ne','0898ca']:
        ...
        df.loc[df['form'] == 'Paldean Form Combat Breed', 'dex_no'] = df['dex_no'].str[0:4] + 'pc'
        df.loc[df['form'] == 'Paldean Form Blaze Breed', 'dex_no'] = df['dex_no'].str[0:4] + 'pb'
        df.loc[df['form'] == 'Paldean Form Aqua Breed', 'dex_no'] = df['dex_no'].str[0:4] + 'pa'
        ...
        df.loc[df['form'] == 'Paldean Form', 'dex_no'] = df['dex_no'].str[0:4] + 'p'
        ...
        save_xlsx('pkmn_g9_pkmn.xlsx')
        df.to_sql('pkmn', engine, index=False)
        #df.to_csv('pkmn_g9_pkmn.csv', index=False)
