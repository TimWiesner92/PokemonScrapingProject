...
class PkmnSpider(scrapy.Spider):
    ...
    def closed(self, data):
        ...
        condition_mover('level up in a Magnetic Field area','location','Chargestone Cave')
        condition_mover('level up near an Icy Rock','location','Twist Mountain (basement)')
        condition_mover('level up near a Mossy Rock','location','Pinwheel Forest (southwest)')
        ...
        df.loc[df['location'] == 'Chargestone Cave', ['method','item_used']] = ['level','']
        df.loc[df['location'] == 'Twist Mountain (basement)', ['method','item_used']] = ['level','']
        df.loc[df['location'] == 'Pinwheel Forest (southwest)', ['method','item_used']] = ['level','']
        ...
        df = df[df['evolves_from'].between('0001', '0649') & ~df['evolves_from'].str.match(r'\d{4}[a-z]$')]
        df = df[df['evolves_to'].between('0001', '0649') & ~df['evolves_to'].str.match(r'\d{4}[a-z]$')]
        ...
        save_xlsx('pkmn_g5_evo.xlsx')
        df.to_sql('evo', engine, index=False)
        #df.to_csv('pkmn_g5_evo.csv', index=False)
