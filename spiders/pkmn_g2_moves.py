...
class PkmnSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.serebii.net/attackdex-gs/"]
    ...
    def parse_move_page(self, response):
        ...
        entry = {
            ...
            'accuracy': int(response.css('td.cen::text')[5].get().replace('\r\n\t\t','').replace('\t','')),
    ...
    def closed(self, data):
        ...
        save_xlsx('pkmn_g2_moves.xlsx')
        df.to_sql('moves', engine, index=False)
        #df.to_csv('pkmn_g2_moves.csv', index=False)
