...
class PkmnSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.serebii.net/pokedex-sm/"]

    def parse(self, response):
        '''Method for extracting links to pages to crawl from the pulldowns'''
        pulldown_links = [x for x in response.xpath('//select[@name="SelectURL"]')[0:7].css('option').xpath('@value').getall() if x != '#']
    ...
    def parse_move_page(self, response):
        ...
        def extract_types(html_string, pat1, pat2, pk_name):
            ...
            elif form_match == 'Normal':
                form_match = 'Normal Form'
        ...
        if 'Alola Form' in evs_html:
            evs_html = 'Normal Form</b><br>' + evs_html
        ...
        mega_primal_forms = list(set(response.xpath('//td[@class="fooevo" and @colspan="6"]//b//text()').getall()))
        ...
        if len(mega_primal_forms) > 0:
            ...
            if len(mega_primal_forms) == 1 and mega_primal_forms != ['Mega Evolution']:
                if mega_primal_forms == ['Ultra Burst']:
                    forms = response.xpath('//td[text()="Alternate Forms"]/ancestor::tr/following-sibling::tr[1]//td/b/text()').getall() + mega_primal_forms
                    base_types = [extract_types(html,p1,p2,name)[1] for html in types_html]
            ...
            elif len(mega_primal_forms) == 1:
                mp_types = [[i.replace('/pokedex-bw/type/','').replace('.gif','').capitalize() for i in response.xpath('//table[@class="dextable"][.//td/h2/font/b[contains(text(), "' + mega_primal_forms[0] + '")]]/following-sibling::table[1]//td[text()="Type"]/ancestor::tr/following-sibling::tr[1]//td[5]//img[starts-with(@src, "/pokedex-bw/type/")]/@src').getall()]]
                mp_height_raw = response.xpath('//table[@class="dextable"][.//td/h2/font/b[contains(text(), "' + mega_primal_forms[0] + '")]]/following-sibling::table[1]//td[text()="Height"]/ancestor::tr/following-sibling::tr[1]//td[2]//text()').get().replace('"','').replace(' / ','\'').split('\'')
                mp_height = [round((float(mp_height_raw[0]) + (float(mp_height_raw[1]) / 12)),2)]
                mp_weight = [float(response.xpath('//table[@class="dextable"][.//td/h2/font/b[contains(text(), "' + mega_primal_forms[0] + '")]]/following-sibling::table[1]//td[text()="Weight"]/ancestor::tr/following-sibling::tr[1]//td[3]//text()').get().replace(',','').replace('lbs',''))]
            else:
                mp_types = [[i.replace('/pokedex-bw/type/','').replace('.gif','').capitalize() for i in response.xpath('//table[@class="dextable"][.//td[font[@size="3"]/b[text()="' + mega_primal_forms[j] + '"]]]/following-sibling::table[1]//td[text()="Type"]/ancestor::tr/following-sibling::tr[1]//td[5]//img[starts-with(@src, "/pokedex-bw/type/")]/@src').getall()] for j in range(len(mega_primal_forms))]
                mp_height = [round((float(i[0]) + (float(i[1]) / 12)),2) for i in [response.xpath('//table[@class="dextable"][.//td[font[@size="3"]/b[text()="' + mega_primal_forms[j] + '"]]]/following-sibling::table//td[text()="Height"]/ancestor::tr/following-sibling::tr[1]//td[2]//text()').get().replace('"','').replace(' / ','\'').split('\'') for j in range(len(mega_primal_forms))]]
                mp_weight = [float(response.xpath('//table[@class="dextable"][.//td[font[@size="3"]/b[text()="' + mega_primal_forms[j] + '"]]]/following-sibling::table//td[text()="Weight"]/ancestor::tr/following-sibling::tr[1]//td[3]//text()').get().replace(',','').replace('lbs','')) for j in range(len(mega_primal_forms))]
        ...
        for form in forms:
            ...
            # base_stats:
            base_stats = [int(i) for i in response.xpath('(//table[@class="dextable"][.//td[h2[starts-with(text(), "Stats")]]])[' + str(stat_sel) + ']//td[@class="fooinfo" and contains(text(), "Base Stats")]/following-sibling::td/text()').getall()]
            if len(base_stats) == 0:
                base_stats = [int(i) for i in response.xpath('(//table[@class="dextable"][.//td[h2[b[starts-with(text(), "Stats")]]]])[' + str(stat_sel) + ']//td[@class="fooinfo" and contains(text(), "Base Stats")]/following-sibling::td/text()').getall()]
...
#                'ability_two': abilities[1] if len(abilities) == 2 else '',
                ...
                'egg_group_one': eg1,
                'egg_group_two': eg2,
                'sos_calling': response.xpath('//td[text()="S.O.S. Calling"]/ancestor::tr/following-sibling::tr[1]//td[4]//text()').get().strip(),
                'hp_evs': evs_sel[0],
                'atk_evs': evs_sel[1],
                'def_evs': evs_sel[2],
    ...
    def closed(self, data):
        ...
        df.loc[df['form'] == "Normal Form", 'dex_no'] = df['dex_no'].str[0:4]
        ...
        save_xlsx('pkmn_g7_pkmn.xlsx')
        df.to_sql('pkmn', engine, index=False)
        #df.to_csv('pkmn_g7_pkmn.csv', index=False)
