...
class PkmnSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.serebii.net/attackdex-swsh/"]
    ...
    def parse_move_page(self, response):
        ...
        # returns 25 errors, corresponding to moves exclusive to Legends: Arceus
        entry = {
            'move_name': response.xpath('(//td[@class="cen"])[1]/text()').get().strip(),
            'battle_type': response.xpath('(//td[@class="cen"])[2]//img[starts-with(@src, "/pokedex-bw/type/")]/@src').get().replace('/pokedex-bw/type/','').replace('.gif','').capitalize(),
            'category': response.xpath('(//td[@class="cen"])[3]//img[starts-with(@src, "/pokedex-bw/type/")]/@src').get().replace('/pokedex-bw/type/','').replace('.png','').capitalize(),
            ...
            'crit_rate': response.xpath('//td[b[text()="Base Critical Hit Rate"]]/ancestor::tr/following-sibling::tr[1]//td[@class="cen"][1]/text()').get().strip(),
            ...
            'biting move': response.xpath('//td[contains(text(), "Biting Move")]/ancestor::tr/following-sibling::tr[1]//td[@class="cen"][4]/text()').get().strip(),
            'snatchable': response.xpath('//td[contains(text(), "Sound-Type")]/ancestor::tr/following-sibling::tr[1]//td[@class="cen"][5]/text()').get().strip(),
            'affected_by_gravity': response.xpath('//td[contains(text(), "Affected by Gravity")]/ancestor::tr/following-sibling::tr[1]//td[@class="cen"][1]/text()').get().strip(),
            'defrosts_when_used': response.xpath('//td[contains(text(), "Defrosts When Used?")]/ancestor::tr/following-sibling::tr[1]//td[@class="cen"][2]/text()').get().strip(),
    ...
    def closed(self, data):
        ...
        save_xlsx('pkmn_g8_moves.xlsx')
        df.to_sql('moves', engine, index=False)
        #df.to_csv('pkmn_g8_moves.csv', index=False)
