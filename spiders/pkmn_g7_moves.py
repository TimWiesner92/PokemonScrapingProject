...
class PkmnSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.serebii.net/attackdex-sm/"]
    ...
    def parse_move_page(self, response):
        ...
        entry = {
            ...
            'power_points': int(response.xpath('(//td[@class="cen"])[4]//text()').get().strip().replace(' (SMUSUM)','')),
            'base_power': int(response.xpath('(//td[@class="cen"])[5]//text()').get().strip().replace(' (SMUSUM)','')),
            ...
            'z_move': response.xpath('//td[contains(text(), "Z Move")]/ancestor::tr/following-sibling::tr[1]//td[@class="cen"][5]/text()').get().strip(),
    ...
    def closed(self, data):
        ...
        save_xlsx('pkmn_g7_moves.xlsx')
        df.to_sql('moves', engine, index=False)
        #df.to_csv('pkmn_g7_moves.csv', index=False)
